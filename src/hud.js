import { CONFIG, ROLE_LABELS, ACTIVITY_LABELS, STATE_LABELS } from './config.js';
import { Sim, agents, animals, CameraState, Follow } from './state.js';
...
import { Lamps, Deposits } from './world.js';
import { POLICY_LABELS } from './config.js';
...
const ROLE_ORDER = ['agricultor', 'comerciante', 'minero', 'lenador', 'aldeano', 'nino', 'clerigo', 'guardia', 'curandero', 'viajero'];
...
export const HUD = {
  ...
  init(callbacks) {
    ...
    this.els = {
      hud: $('hud'), toggle: $('hud-toggle'),
      time: $('hud-time'), day: $('hud-day'), phase: $('hud-phase'), season: $('hud-season'),
      weather: $('hud-weather'), weatherSub: $('hud-weather-sub'), icon: $('hud-weather-icon'),
      stats: $('hud-stats'), resources: $('hud-resources'), town: $('hud-town'), log: $('hud-log'),
      tech: $('hud-tech'), techBar: $('hud-tech-bar'), ruler: $('hud-ruler'),
  ...
  // Gráfica de líneas en SVG: población, grano y enfermos de los últimos días.
  drawChart() {
    ...
    this.els.chartLegend.innerHTML = `<span style="color:#C9A227">Población ${last.pop}</span><span style="color:#7FAE68">Grano ${last.grano}</span><span style="color:#D06A5A">Enfermos ${last.sick}</span>`;
  },
  ...
  updateAgentPanel() {
    ...
    if (a.sick) lines.push(`<p class="agent-warn">Está ${a.isFemale ? 'enferma' : 'enfermo'}</p>`);
  ...
  update(realDt) {
    ...
    e.season.textContent = `${DayCycle.season}, día ${DayCycle.dayOfSeason} de ${CONFIG.calendar.daysPerSeason}, año ${DayCycle.year}`;
    ...
    let sheltered = 0, hunger = 0, energy = 0, mood = 0, health = 0, residents = 0, sleeping = 0, sick = 0;
    for (const a of agents) {
      counts[a.role] = (counts[a.role] || 0) + 1;
      if (a.state === STATE.SHELTERED) sheltered++;
      if (a.sleeping) sleeping++;
      if (a.role !== 'viajero') {
        residents++; hunger += a.needs.hunger; energy += a.needs.energy; mood += a.needs.mood; health += a.needs.health;
        if (a.sick) sick++;
      }
    }
    const rows = [];
    for (const r of ROLE_ORDER) if (counts[r] > 0 || r === 'aldeano') rows.push([ROLE_LABELS[r], counts[r], r === 'nino' || r === 'clerigo' ? 'optional' : '']);
    const pct = (v) => Math.round(v * 100) + '%';
    let livestock = 0;
    for (const an of animals) if (!an.removed) livestock++;
    rows.push(['Total', agents.length, 'total'], ['Animales', livestock, ''], ['Refugiados', sheltered, ''], ['Durmiendo', sleeping, 'optional']);
    ...
    const techRows = [
      ['Estudio', t ? t.label : 'todo descubierto', ''],
      ['Avances', `${Tech.unlocked.size} / ${CONFIG.tech.tree.length}`, ''],
      ['Saber al día', Tech.rate().toFixed(1), 'optional']
    ];
    this.rows(e.tech, techRows);
    e.techBar.style.width = Math.round(Tech.progress() * 100) + '%';
    const rulerRows = [
      ['Señor', Ruler.name, ''],
      ['Popularidad', pct(Ruler.popularity), ''],
      ['Tesoro', Math.round(Economy.treasury), ''],
      ['Decreto', Ruler.policy ? POLICY_LABELS[Ruler.policy].split(':')[0].replace('Decreto de ', '') : 'ninguno', ''],
      ['Faroles', Lamps.list.length, 'optional'],
      ['Yacimientos', `${World.deposits.length} / ${Deposits.list.length}`, '']
    ];
    this.rows(e.ruler, rulerRows);
    const town = [
      ['Vivienda', `${residents} / ${Growth.housingCapacity()}`, ''],
      ['Hambre media', pct(residents ? hunger / residents : 0), ''],
      ['Energía media', pct(residents ? energy / residents : 0), 'optional'],
      ['Ánimo medio', pct(residents ? mood / residents : 0), ''],
      ['Salud media', pct(residents ? health / residents : 0), ''],
      ['Enfermos', sick, ''],
